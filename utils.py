...
class DataLoader:
    ...
    def load_data(self, data_path, index_dir):
        ...
        for review in reviews:
            ...
            """
            tem_encode = self.tokenizer(tem)
            tokens = tem_encode['input_ids']
            mask = tem_encode['attention_mask']
            pad_token = self.tokenizer.pad_token_id
            lengths = len(tokens)
            text_tokens = tokens[:self.seq_len] if lengths > self.seq_len else tokens + [pad_token] * (
                    self.seq_len - lengths)
            text_mask = mask[:self.seq_len] if lengths > self.seq_len else mask + [0] * (self.seq_len - lengths)
            all_tokens = [self.tokenizer.bos_token_id] + text_tokens + [self.tokenizer.eos_token_id]
            all_mask = [1] + text_mask + [1]

            
            if lengths > self.seq_len:
                all_tokens = [self.tokenizer.bos_token_id] + tokens[:self.seq_len] + [self.tokenizer.eos_token_id]
                all_mask = [1] + mask[:self.seq_len] + [1]
            else: 
                all_tokens = [self.tokenizer.bos_token_id] + tokens + [self.tokenizer.eos_token_id] + [pad_token] * (self.seq_len - lengths)
                all_mask = [1] + mask + [1] + [0] * (self.seq_len - lengths)
            """
